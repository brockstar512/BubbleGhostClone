Shader "AllIn1SpriteShader/AllIn1Urp2dRenderer"
{
    Properties
    {
    	...
    	_MaskTex("Lighting Mask", 2D) = "white" {} //164
		_LitAmount("Lit Amount",  Range(0,1)) = 1 //165
		_NormalMap("Normal Map", 2D) = "bump" {} //166
		_NormalStrength("Normal Strength",  Range(0,10)) = 1 //167
		[Toggle()] _GlowAffectLight("Glow affects light", Float) = 1 //168
    ...
    SubShader
    {
        ...
        Pass //Main Light Shapes pass
        {
			...
			#pragma shader_feature GLOWLIGHT_ON
		...
		Pass //Normal Mapping
        {
            ...
            sampler2D _MainTex, _MaskTex, _NormalMap;
            half4 _MainTex_ST, _MainTex_TexelSize, _Color, _NormalMap_ST;
			half _Alpha, _NormalStrength;
            float _RandomSeed;
            ...
            v2f NormalsRenderingVertex(appdata v)
            {
                ...
                o.normalWS = TransformObjectToWorldDir(half3(0, 0, -1));
            ...
            float4 NormalsRenderingFragment(v2f i) : SV_Target
            {
				...
				half4 normalSample = tex2D(_NormalMap, i.uv);
            	
				#if BLUR_ON
				#if !BLURISHD_ON
				normalSample = BlurHD(i.uv, _NormalMap, _BlurIntensity, 1, 1);
				#else
				normalSample = Blur(i.uv, _NormalMap, _BlurIntensity);
				#endif
				#endif
				...
				#define rot(n) mul(n, half2x2(cos(_MotionBlurAngle), -sin(_MotionBlurAngle), sin(_MotionBlurAngle), cos(_MotionBlurAngle)))
				_MotionBlurDist = _MotionBlurDist * 0.005;
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(-_MotionBlurDist, -_MotionBlurDist)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(-_MotionBlurDist * 2, -_MotionBlurDist * 2)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(-_MotionBlurDist * 3, -_MotionBlurDist * 3)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(-_MotionBlurDist * 4, -_MotionBlurDist * 4)));
				normalSample.rgb += tex2D(_NormalMap, i.uv);
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(_MotionBlurDist, _MotionBlurDist)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(_MotionBlurDist * 2, _MotionBlurDist * 2)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(_MotionBlurDist * 3, _MotionBlurDist * 3)));
				normalSample.rgb += tex2D(_NormalMap, i.uv + rot(half2(_MotionBlurDist * 4, _MotionBlurDist * 4)));
	...
	CustomEditor "AllIn12DRendererMaterialInspector"
